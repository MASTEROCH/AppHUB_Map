...
function runZAnim(){stopAnim();let last=performance.now();
  const step=now=>{const dt=Math.min(64,now-last);last=now;const e=RM?1:1-Math.pow(1-.22,dt/16.7);
...
function panTo(cx,cy,scale,yf,xoff,dur){const rect=svg.getBoundingClientRect();
  ...
  const k=clampK(scale);tweenTo(k,pt.x-k*cx,pt.y-k*cy,dur==null?720:dur);}
...
function fitWidthTop(dur){const v=svg.viewBox.baseVal,rect=svg.getBoundingClientRect(),m=Math.min(rect.width/v.width,rect.height/v.height);
  const k=clampK((rect.width*0.97)/(m*v.width));const pt=svgPt({clientX:rect.left+rect.width/2,clientY:rect.top+10});tweenTo(k,pt.x-k*(v.x+v.width/2),pt.y-k*v.y,dur==null?720:dur);}

...
const gFx=ex("g",{id:"fx"});vp.appendChild(gFx);   // прицел и эффекты — над узлами
...
function applyHighlight(id,fx){const conn=new Set([id]);lowerLinks();let i=0;
  linkEls.forEach(le=>{const on=le.a===id||le.b===id;if(on)raiseLink(le);
    ...
    if(on){conn.add(le.a);conn.add(le.b);if(fx&&!RM)drawBeam(le.glow,i++);}});
  Object.entries(nodeEls).forEach(([nid,g])=>{g.style.opacity=conn.has(nid)?1:.26;g.classList.toggle("sel",nid===id);
    if(fx&&!RM&&nid!==id&&conn.has(nid)){g.classList.remove("ping");void g.getBBox();g.style.animationDelay=(120+([...conn].indexOf(nid))*45)+"ms";g.classList.add("ping");}});
  return conn;}
/* луч: свечение связи рисуется от узла к узлу (dashoffset по длине пути), ядро-пунктир течёт поверх */
function drawBeam(el,i){try{const L=el.getTotalLength();el.style.transition="none";el.style.strokeDasharray=L+" "+L;el.style.strokeDashoffset=L;void el.getBoundingClientRect();
  el.style.transition=`stroke-dashoffset .75s cubic-bezier(.16,1,.3,1) ${Math.min(i,14)*35}ms`;el.style.strokeDashoffset=0;
  setTimeout(()=>{el.style.transition="";el.style.strokeDasharray="";el.style.strokeDashoffset="";},1200+i*35);}catch(e){}}
/* прицел: четыре уголка и тонкая рамка сжимаются вокруг выбранного узла */
function drawReticle(id){gFx.innerHTML="";const n=N[id];if(!n)return;const w=nodeW(n)+18,h=nodeH(n)+18,x=n.x-w/2,y=n.y-h/2,c=C[n.layer]||C.L2,L=Math.min(18,w/4);
  const g=ex("g",{class:"ret"+(RM?" still":""),"pointer-events":"none"});
  g.appendChild(ex("rect",{class:"retbox",x,y,width:w,height:h,rx:16,fill:"none",stroke:c,"stroke-width":1,"stroke-dasharray":"3 6",opacity:.55}));
  [[x,y,1,1],[x+w,y,-1,1],[x,y+h,1,-1],[x+w,y+h,-1,-1]].forEach(([px,py,sx,sy])=>g.appendChild(ex("path",{d:`M${px},${py+sy*L} L${px},${py} L${px+sx*L},${py}`,fill:"none",stroke:c,"stroke-width":2.2,"stroke-linecap":"round"})));
  const t=ex("text",{class:"retlbl",x:x+w+8,y:y+10,fill:c});t.textContent=(LBADGE[n.layer]||n.layer)+" · "+(n.s==="live"?"LIVE":(n.s==="dev"?"DEV":(n.s==="core"?"CORE":"IDEA")));g.appendChild(t);
  gFx.appendChild(g);}
function clearReticle(){gFx.innerHTML="";}
...
let viewMode="focus";
function select(id){if(!N[id])return;const fresh=selectedId!==id;if(fresh){heroView="client";viewMode="focus";}selectedId=id;clearSubs();applyHighlight(id,fresh);if(N[id].subs)showSubs(id);
  if(EDITABLE&&editMode)renderEdit(id);else renderInfo(id);
  panel.classList.add("open");document.body.classList.add("sel-open");
  if(!(EDITABLE&&editMode)){if(viewMode==="links")fitConnections(id);else focusSelected(id);drawReticle(id);}
  try{history.replaceState(null,"","#"+id);}catch(e){location.hash=id;}}
function reset(){const had=selectedId;selectedId=null;clearSubs();baseLinks();clearReticle();
...
function renderInfo(id){const n=N[id],c=C[n.layer]||C.L2,st=stOf(n);
  ...
  ${(()=>{const n2=connectedIds(id).length-1;return n2?`<div class="viewseg"><button data-view-mode="focus" class="${viewMode==="focus"?"on":""}"><i>◎</i>Фокус</button><button data-view-mode="links" class="${viewMode==="links"?"on":""}"><i>⌘</i>Связи · ${n2}</button></div>`:"";})()}
...
function runZAnim(){stopAnim();let last=performance.now();
  const step=now=>{const dt=Math.min(64,now-last);last=now;const e=RM?1:1-Math.pow(1-.34,dt/16.7);   // резче: .34 за кадр 60 Гц (Роч: «было заторможенно»)
    // масштаб и сдвиг идут ОДНИМ коэффициентом: тогда точка под курсором стоит на месте на каждом кадре
    // (лог-интерполяция k при линейном tx давала «отъезд перед подлётом» — Роч заметил)
    zk+=(tk-zk)*e;ztx+=(ttx-ztx)*e;zty+=(tty-zty)*e;
...
svg.addEventListener("pointerdown",e=>{e.preventDefault();ptrs.set(e.pointerId,e);cancelZAnim();
  ...
  if(EDITABLE&&editMode&&ng){const id=ng.dataset.id;const p=svgPt(e);nodeDrag={id,ox:N[id].x,oy:N[id].y,px:p.x,py:p.y};moved=false;clearReticle();}
...
window.addEventListener("keydown",e=>{
  ...
  if((e.key==="c"||e.key==="C"||e.key==="с"||e.key==="С")&&selectedId&&!/INPUT|TEXTAREA|SELECT/.test(document.activeElement?.tagName||"")){e.preventDefault();setViewMode(viewMode==="links"?"focus":"links");return;}
...
panel.addEventListener("click",e=>{
  ...
  const vm=e.target.closest("[data-view-mode]");if(vm){setViewMode(vm.dataset.viewMode);return;}
...
function focusSelected(id){const n=N[id];if(!n)return;const mob=isMob();
  if(PORTRAIT){panTo(MOBC,n.y,portraitK(),0.22);return;}
  panTo(n.x,n.y,n.t==="hub"?(mob?1.6:1.5):(mob?2.2:2.1),mob?0.22:0.48,mob?0:204,640);}
/* видимая область холста: десктоп минус панель справа, телефон — над шторкой */
function viewArea(){const r=svg.getBoundingClientRect();if(isMob())return{x:r.left,y:r.top+8,w:r.width,h:r.height*0.40-8};return{x:r.left+12,y:r.top+12,w:r.width-420,h:r.height-24};}
function connectedIds(id){const s=new Set([id]);L.forEach(([a,b])=>{if(a===id&&N[b])s.add(b);if(b===id&&N[a])s.add(a);});return[...s];}
/* отъезд ровно на масштаб, где видны узел и все его связи */
function fitConnections(id){const ids=connectedIds(id);if(ids.length<2){focusSelected(id);return;}
  let a=1e9,b=1e9,c=-1e9,d=-1e9;ids.forEach(k=>{const n=N[k],w=nodeW(n),h=nodeH(n);a=Math.min(a,n.x-w/2);c=Math.max(c,n.x+w/2);b=Math.min(b,n.y-h/2);d=Math.max(d,n.y+h/2);});
  const pad=56,bw=c-a+pad*2,bh=d-b+pad*2,A=viewArea(),v=svg.viewBox.baseVal,r=svg.getBoundingClientRect(),m=Math.min(r.width/v.width,r.height/v.height);
  const k=Math.max(.45,Math.min(isMob()?2.6:2.4,Math.min(A.w/(m*bw),A.h/(m*bh))));
  const pt=svgPt({clientX:A.x+A.w/2,clientY:A.y+A.h/2});tweenTo(k,pt.x-k*(a+c)/2,pt.y-k*(b+d)/2,680);}
function setViewMode(m){if(!selectedId)return;viewMode=m;if(m==="links")fitConnections(selectedId);else focusSelected(selectedId);
  $$("#panel [data-view-mode]").forEach(b=>b.classList.toggle("on",b.dataset.viewMode===m));}

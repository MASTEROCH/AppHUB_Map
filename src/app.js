...
function comet(d,color,dur,delay,r){const mp=ex("path",{d,fill:"none",stroke:"none"});const id="cm"+(cometN++);mp.id=id;gLoop.appendChild(mp);
  const c=ex("circle",{r:r||4,fill:color,opacity:.95});
  ...
  am.appendChild(mpath);c.appendChild(am);gLoop.appendChild(c);}

/* ——— render ——— */
function render(){
  gLoop.innerHTML="";gZ.innerHTML="";gL.innerHTML="";gN.innerHTML="";linkEls=[];nodeEls={};cometN=0;clearSubs();
  if(N.l1&&N.l3){
    const d=`M${N.l3.x},${N.l3.y+62} C${N.l3.x+40},760 ${N.l1.x-40},760 ${N.l1.x},${N.l1.y+62}`;
    gLoop.appendChild(ex("path",{d,fill:"none",stroke:"url(#loop)","stroke-width":11,opacity:.12}));
    gLoop.appendChild(ex("path",{class:"flowline",d,fill:"none",stroke:"url(#loop)","stroke-width":2.4,opacity:.6,"stroke-linecap":"round"}));
    comet(d,"#C5FF5F",4.5,0,4.5);comet(d,"#C5FF5F",4.5,2.2,4.5);
...
function buildChrome(){
  ...
  if(chipsEl){
    ...
    chipsEl.addEventListener("click",e=>{const b=e.target.closest(".chip");if(!b)return;filter=b.dataset.f;$$("#chips .chip").forEach(x=>x.classList.toggle("on",x===b));applyFilter();});}
...
function focusNode(id,scale){const v=svg.viewBox.baseVal,n=N[id];if(!n)return;const tx=v.x+v.width/2,ty=v.y+v.height*0.34;zk=scale||1.6;ztx=tx-zk*n.x;zty=ty-zk*n.y;applyZ();}
function focusAll(sm){if(sm){vp.classList.add("smooth");setTimeout(()=>{if(tourIdx<0)vp.classList.remove("smooth");},740);}
  if(window.innerWidth<=760){const v=svg.viewBox.baseVal,cx=v.x+v.width/2,cy=v.y+v.height/2;zk=1.85;ztx=cx*(1-zk);zty=cy*(1-zk);}else{zk=1;ztx=0;zty=0;}applyZ();}
function focusSelected(id){const n=N[id];if(!n)return;const rect=svg.getBoundingClientRect();
  const wide=window.innerWidth>760,drawerW=wide?408:0;
  const pt=svgPt({clientX:rect.left+(rect.width-drawerW)/2,clientY:rect.top+rect.height*(wide?0.46:0.12)});
  zk=n.t==="hub"?1.35:1.75;ztx=pt.x-zk*n.x;zty=pt.y-zk*n.y;
  vp.classList.add("smooth");applyZ();setTimeout(()=>{if(tourIdx<0)vp.classList.remove("smooth");},740);}
...
function tourGo(i){if(i<0)return;if(i>=TOUR.length){endTour();return;}tourIdx=i;const s=TOUR[i];
  ...
  if(s.focus==="all")focusAll();else if(s.focus)focusNode(s.focus,s.scale);
